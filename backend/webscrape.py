...
def search_player(player):
    ...
    img = soup.find_all('source')[0].find('source')['srcset'].split(',')[1][:-2]
    ...
    return [int(round(float(td_tags[-1].text))), int(opponent), file, img, position.upper()]
